...
public class StaffController {
  ...
  public ResponseEntity<BaseResponse<Void>> requestApproval(@AuthenticationPrincipal GatewayUserPrincipal user, @RequestBody StaffsendApprovalRequest request) {
  ...
  public ResponseEntity<BaseResponse<OwnerApproveResponse>> approveStaff(@RequestBody OwnerApproveRequest request, @AuthenticationPrincipal GatewayUserPrincipal user) {

    Long storeId = user.getStoreId();

    log.info("[사장님 승인] 사장님 승인 요청 - staffId: {}, storeId: {}", request.getStaffId(), storeId);

    // 서비스에서 상태 반환받기 (storeId는 JWT 클레임 출처 - 게이트웨이 헤더)
    Status resultStatus = staffService.approveStaff(request.getStaffId(), request.getStatus(), storeId);
