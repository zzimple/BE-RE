...
public class OwnerController {
  ...
  public ResponseEntity<BaseResponse<List<MonthlySalesAvgResponse>>> getMonthlyAverageSales(
    ...
    // storeId는 게이트웨이가 JWT 클레임에서 추출해 전달 (기존: userId로 Store 재조회)
    List<MonthlySalesAvgResponse> result = ownerService.getMonthlyAverageSales(user.getStoreId());
  ...
  public ResponseEntity<BaseResponse<List<WeeklySalesSimpleResponse>>> getWeeklySales(
    ...
    List<WeeklySalesSimpleResponse> response = ownerService.getWeeklySales(user.getStoreId());
